...
namespace Demographic
{
    public class Person
    {
        private static double _childBirthProbability = 0.151;
        ...
        {
            if (IsAlive && Gender == "Female" && Age >= 18 && Age <= 45)
            {
                if (ProbabilityCalculator.IsEventHappened(_childBirthProbability))
                {
                    string newChildGender = ProbabilityCalculator.IsEventHappened(0.55) ? "Female" : "Male";
